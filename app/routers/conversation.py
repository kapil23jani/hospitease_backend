...
async def complaint_talk_no_session_audio_in(
    ...
    # Track problem and follow-up history
    if "problem_history" not in current_data:
        current_data["problem_history"] = []
    if next_field_to_ask == "problem" and user_input_text:
        current_data["problem_history"].append({"question": current_data.get("last_problem_prompt", ""), "answer": user_input_text})

        # Count follow-ups
        followup_count = len(current_data["problem_history"])
        # If user says yes to log complaint
        if any(x in user_input_text for x in ["हाँ", "yes", "complaint", "शिकायत", "log"]):
            # Move to next field after problem
        ...
        # If less than 2 follow-ups, ask another follow-up or suggestion
        if followup_count < 2:
            prompt = (
                "तुम एक दोस्ताना शिकायत अधिकारी हो। यूज़र ने यह समस्या बताई है: "
                f"\"{current_data['problem_history'][0]['answer']}\" "
                + " ".join([f"फॉलो-अप {i+1}: {item['answer']}" for i, item in enumerate(current_data['problem_history'][1:])])
                + " अगर समस्या स्पष्ट नहीं है, तो और जानकारी के लिए विनम्र follow-up सवाल पूछो। "
                "अगर समस्या स्पष्ट है और सुझाव संभव है, तो व्यावहारिक, सरल हिंदी में समाधान या सुझाव दो।"
            )
            ai_reply = llm.invoke(prompt)
            if hasattr(ai_reply, "content"):
                ai_reply = ai_reply.content
            ai_reply = clean_reply_text(ai_reply)
            ai_reply = sanitize_reply_text(ai_reply)
            ai_reply = safe_tts_text(ai_reply)
            current_data["last_problem_prompt"] = ai_reply
            audio_url = tts(ai_reply, base_url=base_url)
            return {
                "done": False,
                "prompt": ai_reply,
                "audio_url": audio_url,
                "next_field_to_ask": "problem",
                "current_data": current_data
            }
        else:
            # After 2 follow-ups, ask if user wants to log complaint
            ask_log = "अगर आप सुझाव से संतुष्ट नहीं हैं और शिकायत दर्ज करना चाहते हैं, तो कृपया 'हाँ' कहें, अन्यथा 'नहीं' कहें।"
            audio_url = tts(ask_log, base_url=base_url)
            current_data["last_problem_prompt"] = ask_log
            return {
                "done": False,
                "prompt": ask_log,
                "audio_url": audio_url,
    ...
    if next_field_to_ask and user_input_text and next_field_to_ask != "problem":

...
from app.crud.staffs import create_staff, get_staff, get_all_staff, update_staff, delete_staff
...
async def read_all_staff(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    return await get_all_staff(db, skip, limit)

@router.put("/{staff_id}", response_model=StaffOut)
async def update_staff(staff_id: int, staff: StaffUpdate, db: AsyncSession = Depends(get_db)):

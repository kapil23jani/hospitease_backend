...
from app.crud.appointment import create_appointment, get_appointments, get_appointment_by_id, update_appointment, delete_appointment, get_appointments_by_doctor_id, get_appointments_by_patient_id
...
async def list_appointments(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
...
async def list_appointments_by_doctor(doctor_id: int, skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
    return await get_appointments_by_doctor_id(db, doctor_id, skip, limit)

@router.get("/patient/{patient_id}", response_model=List[AppointmentResponse])
async def list_appointments_by_patient(patient_id: int, skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
